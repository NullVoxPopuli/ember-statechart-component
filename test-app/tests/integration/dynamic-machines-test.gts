...
//       const value = (x: any) => x?.getSnapshot?.()?.value;
//
//       await render(
//         <template>
//           <ParentMachine as |machine|>
//             <div id="parent">{{machine.statePath}}</div>
//             <div id="child">{{value machine.snapshot.children.timer}}</div>
//
//             <button type="button" {{on 'click' (fn machine.snapshot.children.timer.send declareDone)}}>
...
//       const getChild = (x) => x.snapshot?.children['child-machine'];
//
//       await render(
//         <template>
//           <parentMachine as |x|>
//         {{log x.actor.system}}
//
//             {{#if (call x.snapshot x.snapshot.matches 'idle')}}
//               <button id="invoke-child" type="button" {{on 'click' (fn x.send invokeChild)}}>Invoke Child</button>
//             {{else}}
//               <button id='reset' type="button" {{on 'click' (fn x.send reset)}}>Reset</button>
//
//               <button id="update-context" type="button" {{on 'click' (fn update x.snapshot)}}>
//                 Update Context
//               </button>
//
//               <out>{{prop x.snapshot}}</out>
//               <div>
//                 debug:<br>
//                 parent: {{x.snapshot.value}}<br>
//                 child: {{getChild x}}
//               </div>
//             {{/if}}
//           </parentMachine>
